...
export function createSuggestionBox({
	...
	if (error.replacements && error.replacements.length > 0) {
		error.replacements.slice(0, 3).forEach((replacement) => {
			...
			const replacementValue = typeof replacement === 'string' ? replacement : replacement.value;
			replaceButton.textContent = replacementValue;
			replaceButton.addEventListener('click', () => {
				if (onReplace) onReplace(replacementValue);
				container.style.opacity = '0';
				container.style.transform = 'translateY(-10px)';
				setTimeout(() => container.remove(), 300);
			});
